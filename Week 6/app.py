...
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{
    os.path.join(current_dir, 'api_database.sqlite3')}"
db = SQLAlchemy()
db.init_app(app)
...
class EnrollmentList(Resource):
    ...
    def get(self, student_id):
        ...
        if not student:
            raise ResourceValidationError(
                400, "ENROLLMENT002", "Student does not exist.")
    ...
    def post(self, student_id):
        ...
        if not course:
            raise ResourceValidationError(
                400, "ENROLLMENT001", "Course does not exist.")

        enrollment = Enrollment.query.filter(
            Enrollment.student_id == student_id, Enrollment.course_id == course_id).first()
        if enrollment:
            raise ResourceValidationError(
                409, "ENROLLMENT004", "Student is already enrolled in the course.")
...
api.add_resource(EnrollmentList, '/api/student/<int:student_id>/course/<int:course_id>',
