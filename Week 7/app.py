...
def get_student_detail(student_id):
    ...
    enrolled_courses = Course.query.join(
        Enrollments).filter(Enrollments.estudent_id == student_id).all()
...
def withdraw_course(student_id, course_id):
    enrollment = Enrollments.query.filter(
        Enrollments.estudent_id == student_id, Enrollments.ecourse_id == course_id).first()
    if enrollment:
...
def update_student(student_id):
    ...
    if request.method == 'GET':
        ...
        for enrolled_course in updated_enrollment:
            course_id = enrolled_course
            new_enrollment = Enrollments(
                estudent_id=student.student_id, ecourse_id=course_id)
            db.session.add(new_enrollment)
...
def course_details(course_id):
    ...
    enrolled_students = Student.query.join(
        Enrollments).filter(Enrollments.ecourse_id == course_id).all()
...
def add_course():
    if request.method == 'GET':
        ...
        course_code = request.form['c_code']
        course_name = request.form['c_name']
        course_description = request.form['c_desc']
